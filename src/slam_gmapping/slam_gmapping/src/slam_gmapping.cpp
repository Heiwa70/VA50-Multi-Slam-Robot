...
void SlamGmapping::init() {
    ...
    double map_update_interval_sec = this->get_parameter("map_update_interval").as_double();
    ...
    minimum_score_ = this->get_parameter("minimum_score").as_double();
    ...
    RCLCPP_INFO(this->get_logger(), "GMapping initialized with frames: base=%s, map=%s, odom=%s",
...
//        std::bind(&SlamGmapping::laserCallback, this, std::placeholders::_1));
    scan_filter_ = std::make_shared<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>
            (*scan_filter_sub_, *buffer_, odom_frame_, 100, node_);  // Increased queue size from 10 to 100
