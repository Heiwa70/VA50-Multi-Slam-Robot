    ...
    def assign_targets_optimized(self):
        ...
        for i, (rx, ry) in enumerate(self.robots):
            for j, (px, py, score, size) in enumerate(self.points_interet):
                if not (0 <= rx < w and 0 <= ry < h and 0 <= px < w and 0 <= py < h):
                    continue
                if not np.isfinite(self.cost_map[py, px]):
                    continue
                try:
                    ...
                    paths_matrix[i][j] = [(c, r) for r, c in path]  # convertir en (x,y)
                except Exception:
                    continue

        # Assignation initiale via Hungarian
        big = 1e9
        cost_matrix_safe = np.where(np.isfinite(cost_matrix), cost_matrix, big)
        ...
        for i in range(n_r):
            for j, (px, py, score, size) in enumerate(self.points_interet):
                if (px, py) in assigned_points or not assigned_points:
                    continue
                dist = min(np.hypot(px - ax, py - ay) for (ax, ay) in assigned_points)
                cost_matrix[i, j] *= (1 + np.exp(-dist/5))
...
class AffectationPointsInterets(Node):
    def __init__(self):
        ...
        self.robot_frames = ["robot1/base_link", "robot2/base_link", "robot3/base_link"]
    ...
    def update_poses(self):
        ...
        for frame in self.robot_frames:
            try:
                ...
                self.positions_robots[frame] = {"x": x, "y": y, "px": px, "py": py, "yaw": yaw}
            except Exception as e:
                self.get_logger().warning(f"TF lookup failed for {frame}: {e}")
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        carte = np.array(msg.data, dtype=np.int8).reshape((height, width))

        det = DetectionPointsInterets(carte, res, 0.3, top_k=50)

        robots = []
        nom_robot = {}
        for i, (key, value) in enumerate(self.positions_robots.items()):
...
def main(args=None):
    ...
    node = AffectationPointsInterets()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
